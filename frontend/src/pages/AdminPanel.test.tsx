...
describe('AdminPanel document backup', () => {
    ...
    it('shows the backup tab and cancels without starting a request', async () => {
        ...
        expect(screen.getByText('Verträge und Rechnungen sichern')).toBeInTheDocument()
    ...
    it('downloads the blob, shows progress and releases the object URL', async () => {
        ...
        await waitFor(() => {
            expect(mockPost).toHaveBeenCalledWith('/admin/backup', undefined, { responseType: 'blob' })
        })
        const runningButton = screen.getByRole('button', { name: 'Sicherung wird erstellt …' })
        ...
        await act(async () => {
            resolveBackup({
                ...
                headers: {
                    'content-disposition': 'attachment; filename="atlas-datensicherung-2026-07-17_12-30-00Z.zip"',
                },
            })
            await Promise.resolve()
        })

        await waitFor(() => expect(screen.getByRole('button', { name: 'Alles sichern' })).toBeEnabled())
        expect(URL.createObjectURL).toHaveBeenCalledWith(expect.any(Blob))
        expect(clickSpy).toHaveBeenCalledOnce()
        expect(downloadedFilename).toBe('atlas-datensicherung-2026-07-17_12-30-00Z.zip')
    ...
    it('shows a clear server error and enables retrying', async () => {
        ...
        mockPost.mockRejectedValueOnce({
            response: { data: { detail: 'Datensicherung ist vorübergehend nicht verfügbar' } },
        })
        const backupButton = await openBackupTab()
