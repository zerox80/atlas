...
Die KI extrahiert erneut aus den Originaldateien, ohne frühere Werte oder den Gesprächskontext anderer Verträge als Vorgabe zu erhalten. Jede Analyseanfrage enthält nur feste Anweisungen und den jeweiligen Dokumentabschnitt. Hauptdokument und zugehörige PDF-Anlagen werden anschließend gemeinsam bewertet. Der Bericht enthält sieben semantische Statuswerte: `CONFIRMED`, `EXPLICIT_CONFLICT`, `NOT_EVIDENCED`, `NEW_INFORMATION`, `AMBIGUOUS`, `DERIVED` und `WRONG_SCOPE`. Positionsnetto, Rechnungsbrutto, Vertragswert, Einzelpreis und wiederkehrende Entgelte sowie Rechnungs-, Liefer-, Vertrags- und Leistungsdaten bleiben getrennt. Belege enthalten Dateiname, Originalseite und Textausschnitt; die serverseitige Prüfung kontrolliert Schema, Belegtext, Beträge und Kündigungsfristen. OCR-Tabellen werden einschließlich ihrer separat gelieferten Inhalte ausgewertet.

Ein Lauf verarbeitet Dokumente in Seitenpaketen (`MISTRAL_REVIEW_SECTION_PAGES=4`, zulässig 1–10). Diese technischen Pakete sind keine inhaltlichen Vertragsbestandteile: Geräte-/Lizenzpreis und Support-/Wartungskosten werden getrennt erfasst und bleiben ihrem Vertrag zugeordnet. Die Oberfläche legt den Lauf mit `POST /ai/reviews` und `{"start": true}` an; Startabsicht und Dokumentliste werden gemeinsam gespeichert. `/ai/reviews/{id}/start` setzt gespeicherte Läufe fort, `/pause` hält sie nach dem bereits laufenden Paket an. Die Steuerung läuft auf dem Server unabhängig vom Browser: F5, Navigation und Schließen der Seite stoppen sie nicht. Nach einem Backend-Neustart nimmt der Dispatcher gespeicherte aktive Läufe wieder auf; unterbrochene Pakete werden als Fehler ausgewiesen und können erneut versucht werden. Die neue Steuertabelle wird beim Backend-Start automatisch angelegt. Vorhandene Läufe starten erst nach ausdrücklichem Fortsetzen.

Nach jedem erfolgreichen Paket wird ein Zwischenstand gespeichert. Bei einem Timeout wird nur das betroffene Paket halbiert und erneut versucht, höchstens bis zur einzelnen Seite. Bereits fertige Seiten werden nicht wiederholt; Originalseitenzahlen und Modell bleiben erhalten. Scheitert auch die einzelne Seite oder tritt ein anderer Fehler auf, wird das Dokument als fehlerhaft markiert und mit dem nächsten fortgefahren. **Fehler erneut versuchen** setzt am letzten erfolgreichen Paket fort. Die UI unterscheidet fertige OCR von der noch laufenden KI-Auswertung, zeigt Wartezeit und Anfragezeitlimit und meldet ausbleibende Server-Lebenszeichen. Lebenszeichen bestätigen die laufende Backend-Verarbeitung, nicht den internen Fortschritt des KI-Anbieters. Die Gesamtseitenzahl umfasst Haupt-PDF und PDF-Anlagen; die Aufschlüsselung ist ausklappbar. Fehlercode, HTTP-Status, Modell und Schemafehler bleiben sichtbar; API-Schlüssel und rohe Provider-Antworten werden nicht ausgegeben. Kleinere Wiederholungspakete können zusätzliche OCR-/Analyse-Kosten verursachen.

Für die semantische Prüfung ist `MISTRAL_USE_OCR=true` erforderlich. Pro Dokument mit Anlagen gelten 32 MiB, pro PDF standardmäßig `MISTRAL_MAX_PDF_PAGES=100`; das OCR-Kontextlimit gilt pro Abschnitt. Das separate Acht-Seiten-Limit `MISTRAL_MAX_IMAGE_PDF_PAGES=8` gilt ausschließlich für den Bildmodus und begrenzt OCR-Prüfungen nicht. Modell-, OCR-Modell- oder Abschnittsgrößenwechsel sowie Änderungen der PDF-Dateien erfordern einen neuen Lauf. Neue Prüfungen und Wiederholungen verursachen Analyse-/gegebenenfalls OCR-Kosten.

Jede OCR-/KI-Anfrage erhält ihr eigenes Zeitlimit, auch bei mehreren Textabschnitten oder einer einmaligen Formatkorrektur. Die Verarbeitungssperre wird mit dem nächsten Schritt verlängert; bloße Lebenszeichen verlängern das Zeitlimit nicht. Erfolgreich erkannter OCR-Text wird im internen Zwischenstand gespeichert und bei kleineren Wiederholungspaketen sowie nach einem Neustart wiederverwendet. Dabei werden Dateifingerabdruck, Modellkonfiguration und Originalseiten geprüft. Nach erfolgreicher KI-Auswertung werden die zugehörigen OCR-Seiten aus dem Zwischenstand entfernt. Ein überholter Timeout-Hinweis verschwindet nach dem nächsten erfolgreichen Paket oder einem endgültigen Fehler.

Auch **HTTP 504 vom KI-Anbieter** löst die Aufteilung des betroffenen Pakets aus. Dieser Gateway-Timeout kann vor dem Atlas-Zeitlimit eintreten; ein höheres `MISTRAL_REQUEST_TIMEOUT_SECONDS` verlängert das Zeitlimit des Anbieters nicht. Die Wiederholung verwendet vorhandenen OCR-Text und behält Modell sowie Reasoning-Stufe bei. Scheitert auch eine einzelne Seite, bleibt `PROVIDER_HTTP_504` mit einer konkreten Erklärung sichtbar und die Prüfung fährt mit dem nächsten Dokument fort.

Nur ausdrückliche Abweichungen desselben Geltungsbereichs gelten als Widerspruch. Fehlende Angaben löschen keine gespeicherten Werte. Mehrdeutige Werte und falsche Geltungsbereiche erhalten keine Übernahme-Checkbox. Rechnungspositionen werden als **Vertragsbestandteile** vorgeschlagen; separate Vertragseinheiten erscheinen ausschließlich als manueller Prüfhinweis bei eigenständigen Vertragsmerkmalen. Es werden keine Verträge automatisch angelegt. Quellenprioritäten sind zentral in `backend/review_comparison.py` definiert.
...
Bei Chat-Antworten werden zusätzlich `finish_reason`, `prompt_tokens`, `completion_tokens` und `total_tokens` protokolliert, sofern der Anbieter diese Werte liefert. `Review response rejected` erklärt vor einem Formatkorrekturversuch, ob die Antwort unvollständig (`incomplete_response`), kein gültiges JSON (`invalid_json`) oder nicht schemakonform (`schema_validation`) war. Bei Schemafehlern erscheinen nur geprüfte Feldpfade und Fehlercodes, keine Eingabewerte. `finish_reason=length` weist auf eine Ausgabelängenbegrenzung hin; die Tokenzahlen allein unterscheiden Reasoning- und Antworttokens nicht. Die Ablehnung nennt Lauf, Eintrag, Originalseiten, Textfragment und Versuch; `retry=True` bedeutet einen weiteren KI-Aufruf für diesen Textabschnitt. Mehrere unterschiedliche `run`-Kennungen können gleichzeitig aktiv sein. Zum Vergleichen über die Oberfläche zusätzliche Läufe pausieren; ein bereits begonnenes Seitenpaket wird noch abgeschlossen.
