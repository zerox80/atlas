...
Ein Lauf verarbeitet Dokumente in Seitenabschnitten (`MISTRAL_REVIEW_SECTION_PAGES=4`, zulässig 1–10). `/ai/reviews/{id}/next` antwortet sofort mit HTTP 202; OCR und Analyse laufen im Backend mit einer eigenen Datenbanksitzung. Nach jedem erfolgreichen Abschnitt wird ein Zwischenstand gespeichert. Die Seite während der Prüfung geöffnet lassen: Nach Pause oder Verlassen wird nur der laufende Abschnitt beendet. Ein Fehler stoppt das betroffene Dokument, die übrigen Dokumente werden weiter geprüft. **Fehler erneut versuchen** setzt bei dessen letztem erfolgreichen Abschnitt fort. Abgelaufene Verarbeitungssperren werden als konkrete Unterbrechung angezeigt, statt dasselbe Dokument endlos zu wiederholen. Die UI zeigt Datei, Seitenfortschritt, Verarbeitungsschritt, Fehlercode, HTTP-Status (sofern vorhanden), Modell und bei ungültigem JSON die betroffenen Schemafelder; API-Schlüssel und rohe Provider-Antworten werden nicht ausgegeben.
